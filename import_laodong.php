...
if(isset($_POST['submit'])){
	...
	if(file_exists('uploads/' . $file_name)){
		transfers_to('import_laodong.php?filename=' . $file_name);
	} else {
		transfers_to('import_laodong.php');
	}
}
?>
<h1><a href="index.php" class="nav-button transform"><span></span></a>&nbsp;Import dữ liệu Lao động</h1>
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" id="laodongform" enctype="multipart/form-data">
	<div class="grid">
		<div class="row cells12">
			...
			<div class="cell colspan2 align-left">
				<a href="templates/laodong-v1.xlsx" class="button"><span class="mif-file-download"> File mẫu</span></a>
			</div>
		</div>
	</div>
</form>
...
<tbody>
	...
	$nganhhoc = new NganhHoc();$coquancongtac = new CoQuanCongTac();$tinhtranglaodong = new TinhTrangLaoDong();
	if($sheetData){
		foreach($sheetData as $key => $value){
			if($key >= 3 && $value['A'] > 0 && $value['B']){
				if($congdan->check_exist_code($value['A'])) {
					...
					$id_quocgia = $quocgia->get_id($value['Z']);
					$thoigianbatdau = $value['AA'];$thoigianketthuc=$value['AB'];
					$id_nganhnghe = $nganhnghe->get_id($value['AC']);
					$id_tinhtranglaodong = $tinhtranglaodong->get_id($value['AD']);
					$coquanlaodong = $value['AE'];
					$diachicoquanlaodong = $value['AF'];
